...
class MortalityTable:
    '''
    Instantiates a life table. Data can be provided in the form of the lx, qx or px. Note that the first value is
    the first age considered in the table.
    The life table will be complete, that is, from age 0 to age w (the last age where lx>0).
    Includes the computation of common biometric functions: lx, px, qx, dx, ex.
    Allows for the computation  of common biometric functions: lx, px, qx, dx, ex for non-integers ages, using methods as Uniform Distribution of Death (udd),
    Constant Force of Mortality (cfm) and Balducci approximation (bal)
    '''
    ...
    def nqx(self, x, n=1, method='udd'):
        '''
        Returns the probability that x dies before age x+t
        :param x: age at beginning
        :param n: period
        :param method: the method used to approximate lx for non-integer x's. Use "udd" for Uniform Distribution of Death, "cfm" for Constant Force of Mortality and "bal" for Balducci
        ...
        l_x_n = self.get_lx_method(x + n, method)
        self.msn.append(f"{n}_q_{x}=1-({l_x_n} / {l_x})")
        return 1 - l_x_n / l_x

    def npx(self, x, n=1, method='udd'):
        '''
        Returns the probability that a life x survives beyond age x+t
        :param x: age at beginning
        :param n: period
        :param method: the method used to approximate lx for non-integer x's. Use "udd" for Uniform Distribution of Death, "cfm" for Constant Force of Mortality and "bal" for Balducci
        :return: probability of x is survives at age x+t
        '''
        if method not in self.__methods:
            return np.nan
        if x < 0:
        ...
        l_x = self.get_lx_method(x, method)
        l_x_n = self.get_lx_method(x + n, method)
        self.msn.append(f"{n}_p_{x}={l_x_n} / {l_x}")
        return l_x_n / l_x

    def t_nqx(self, x, t=1, n=1, method='udd'):
        '''
        Returns the probability that a life x dies survives until age x+t and dies before x+t+n
        :param x: age at beginning
